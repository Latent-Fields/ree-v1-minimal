"""
Residue Trajectory Placement Experiment (MECH-056 / EVB-0039)
...
Key diagnostics:
  1. TRAJECTORY-WIDE mean_intermediate_residue_mass > 0
     (path spread: residue actually distributed across trajectory steps)
  2. TRAJECTORY-WIDE last-quarter harm <= ENDPOINT-ONLY * 1.05
     (trajectory-distributed residue doesn't hurt harm avoidance)

Both must hold for MECH-056 PASS.

The claim is NOT that trajectory-wide placement immediately reduces harm — it tests
the necessary structural precondition: can residue mark intermediate steps?
This enables the higher-level architecture (hippocampal trajectory generation shaped
by the residue field) to be built on a sound foundation.
...
def accumulate_intermediate_residue(
    ...
    trajectory.states: List[Tensor[batch, latent_dim]], length H+1.
      Index 0: initial state (already in agent._current_latent)
      Indices 1..H-1: E2-predicted intermediate states
      Index H: terminal predicted state (handled externally by agent.update_residue)
    ...
    Returns total intermediate residue mass added (before accumulation_rate scaling).
    """
    states = trajectory.states  # List of Tensor[batch, latent_dim]
    H = len(states) - 1         # rollout horizon

    if H < 2:
        # Horizon of 1 has no intermediate states — just initial + terminal
        return 0.0

    total_inter_mass = 0.0
    # Intermediate states: indices 1 .. H-1 (skipping initial=0 and terminal=H)
    for i in range(1, H):
        weight = i / H          # 1/H, 2/H, ..., (H-1)/H — linear ramp
...
def run_episode(
    ...
    (without agent.act()) so we have access to selected_trajectory.states.
    """
    obs = env.reset()
    agent.reset()
    ...
    for _ in range(max_steps):
        ...
        # Low-level pipeline (no grad needed for inference)
        with torch.no_grad():
        ...
        # Snapshot residue cost of selected trajectory BEFORE any residue update
        try:
        ...
        if harm < 0:
            ...
            if condition == "TRAJECTORY-WIDE":
                # Spread residue to intermediate trajectory states (decayed)
                inter_mass = accumulate_intermediate_residue(
                    agent, selected_traj, harm_magnitude
                )
                ep_intermediate_mass += inter_mass
            # Both conditions: update terminal state via standard path
...
def run_experiment(
    ...
    if verbose:
        print("[Residue Trajectory Placement — MECH-056 / EVB-0039]")
        ...
        print("    → residue spreads beyond terminal step (path-spread criterion)")
        print("  Diagnostic 2: TRAJECTORY-WIDE last-Q harm <= ENDPOINT-ONLY last-Q harm * 1.05")
        print("    → trajectory-distributed residue is not harmful")
        print()
    ...
    if verbose:
        ...
        if verdict == "PASS":
            ...
            print("    Residue accumulates at intermediate trajectory positions, not just endpoints.")
            print("    This validates the structural precondition for trajectory-first residue placement.")
            print("    The φ(z) residue field can serve as meaningful terrain for hippocampal rollout generation.")
...
def main():
    parser = argparse.ArgumentParser(
        description="MECH-056: Residue Trajectory Placement experiment"
    )
